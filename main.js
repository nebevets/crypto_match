...
function handleClick(event) {
  ...
  if (
    !$this.hasClass("back") ||
    $this === $firstCoinClicked ||
    ($firstCoinClicked && $secondCoinClicked)
  ) {
    return;
  }
  ...
  if ($firstCoinClicked) {
    ...
    if (
      $firstCoinClicked.attr("symbol") === $secondCoinClicked.attr("symbol")
    ) {
      ++matches;
      ...
      walletBalanceSelector =
        "." + $secondCoinClicked.attr("symbol") + " .balance";
      var $newCoinBalance = $(walletBalanceSelector);
      $newCoinBalance.text(
        calculateNewCoinAmount(accuracy, $newCoinBalance.text())
      );
...
function setTotal() {
  ...
  for (var index = 0; index < $marketPriceSpans.length; index++) {
    ...
    var currentBalance = parseFloat(
      $marketPriceSpans[index].parent().children("span.balance").text()
    );
